...
function NavBar(props: { navStyle: string }) {
  ...
  return (
    <div
      className={cn(
        "w-full h-24 flex items-center justify-around bg-neutral-900	 text-white ",
        props.navStyle
      )}
    >
      <Link href="/">
        <div className="logo py-6"></div>
      </Link>

      <div className="hidden sm:flex sm:gap-4">
        <Link href="/collections">
          <Button className="bg-transparent hover:bg-white hover:text-black  duration-200">
            New Arrivals
          </Button>
        </Link>
        ...
        <div>
          ...
          >
            <Link href="/collections?collection=men">
              {" "}
              <MenuItem onClick={handleClose}>Men Sneakers</MenuItem>
            </Link>
            <Link href="/collections?collection=women">
              {" "}
              <MenuItem onClick={handleClose}>Women Sneakers</MenuItem>
            </Link>
            <Link href="/collections?collection=kids">
              {" "}
              <MenuItem onClick={handleClose}>kids Sneaker</MenuItem>
            </Link>
          </Menu>
        </div>
        <Link href="/contact">
      ...
      <div className="hidden sm:flex items-center justify-between w-fit gap-4 h-12 ">
        ...
        <TemporaryDrawer
          title="YOUR CART"
          opener={<IoCartOutline className="text-2xl cursor-pointer" />}
          contente={<CartPage />}
          closeOnClick={true}
          cartLen={cart.length}
        />
        ...
        {loggedIn === "true" ? (
          <Menubar className="bg-transparent border-none p-0 m-0">
            <MenubarMenu>
              <MenubarTrigger className="p-[5px] active:bg-transparent">
          ...
          <Menubar className="bg-transparent border-none p-0 m-0 active:bg-transparent">
            <MenubarMenu>
              <MenubarTrigger className="p-0 active:bg-transparent">
                <IoMdPerson className="text-2xl cursor-pointer" />
              </MenubarTrigger>
              <MenubarContent>
                <Link href="/login">
                  <MenubarItem className="flex items-center justify-between cursor-pointer">
                    <p>LOGIN</p> <IoLogIn className="text-xl font-bold" />
                  </MenubarItem>
                </Link>
                <Link href="/register">
                  {" "}
                  <MenubarItem className="flex items-center justify-between cursor-pointer">
                    <p>REGISTER</p> <FaUserPlus className="text-xl font-bold" />
                  </MenubarItem>
                </Link>
              </MenubarContent>
            </MenubarMenu>
      ...
      <div className="flex sm:hidden cursor-pointer items-center justify-center gap-6">
        ...
        <TemporaryDrawer
          title=""
          opener={<GiHamburgerMenu className="text-4xl" />}
          contente={<MobileNav />}
          closeOnClick
        />
      </div>
